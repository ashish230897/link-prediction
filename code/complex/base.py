...
class KGEModel(torch.nn.Module):
    ...
    def test(
        ...
        Hits@:math:`k` and Mean Average Precision across all possible tail entities.
        ...
        mean_ranks, reciprocal_ranks, hits_at_k, hits_at_one, precisions = [], [], [], [], []
        for i in arange:
            ...
            hits_at_one.append(rank < 1)

            # Precision calculation
            sorted_indices = torch.cat(scores).to(device=t.device).argsort(descending=True)
            precision_at_k = torch.tensor([(sorted_indices[:j] == t).any().item() for j in range(1, k + 1)],device=t.device).float().mean()
        ...
        hits_at_one = int(torch.tensor(hits_at_one).sum()) / len(hits_at_one)
